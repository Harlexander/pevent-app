...
if (isset($_POST['testimony'])) {
    ...
    $sql = "INSERT INTO testimonies (name,email, testimony) VALUES ('$user[full_name]', '$user[email]', '$testimony')";
...
if (isset($_POST['request'])) {
    ...
    $sql = "INSERT INTO prayer_requests (name,email, request) VALUES ('$user[full_name]', '$user[email]', '$request')";
    $query = mysqli_query($conn, $sql);
}
...
$sql2 = "SELECT COUNT(*) AS total_meetings FROM meetings WHERE host_id = '$user[id]'";
$query2 = mysqli_query($conn, $sql2);
$result = mysqli_fetch_assoc($query2);
$num_of_meetings = $result['total_meetings'];

$sql3 = "SELECT * FROM meetings WHERE host_id = '$user[id]' ORDER BY started_at DESC LIMIT 5";
$query3 = mysqli_query($conn, $sql3);
$meetings = mysqli_fetch_all($query3, MYSQLI_ASSOC);

$sql4 = "SELECT * FROM meetings WHERE host_id = '$user[id]' AND closed_at IS NULL ORDER BY started_at DESC LIMIT 1";
$query4 = mysqli_query($conn, $sql4);
$meeting = mysqli_fetch_assoc($query4);
$meeting_live = $meeting;

$hlsquery = mysqli_query($conn, "SELECT hls FROM live");
$hls = mysqli_fetch_assoc($hlsquery);
...
<body class="text-base text-body-color font-poppins bg-body dark:text-white dark:bg-zink-800 ">
                                ...
                                <div class="card dark:bg-zink-700 overflow-hidden">
                                    <div class="card-body !p-0  bg-[#d4dbf9] dark:bg-blue-500/30">
                                        <div class="flex justify-between">
                                            <img style="height: 140px;" class=" w-full object-cover" src="assets/images/prayernetwork-image.jpeg" alt="">
                                        </div>
                                    </div>
                                    <div class="card-body">
                                        <div class="grid grid-cols-12 space-y-4 md:space-y-0 space-x-2">
                                            <div class="col-span-12 md:col-span-4 -mt-11 ">
                                                ...
                                                <div>
                                                    <h6 class="mt-5 mb-1 text-15 font-medium text-gray-700 dark:text-white"><?= $user['full_name'] ?></h6>
                                                    <p class="text-gray-600 text-13 dark:text-zinc-200">Pastor</p>
                                                </div>
                                            </div>
                                            <div class="col-span-12 md:col-span-4">
                                                <div class="grid grid-cols-2 mt-2">
                                                    <div>
                                                        <p class="text-xs leading-4 mb-2 font-medium text-gray-700 dark:text-white"><?= $href ?>join?code=<?= $user['meeting_code'] ?></p>
                                                        <p class="text-13 font-normal text-gray-600 dark:text-zink-200">My Link</p>
                                                    </div>
                                                </div>
                                                <button type="button" class="py-1 mt-6 border-transparent px-2 text-[11px] btn bg-blue-500 hover:bg-blue-600 text-white" onclick="shareLink()">Share My Link<i class="mdi mdi-share ml-2"></i></button>
                                                <script>
                                                    function shareLink() {
                                                        const link = "<?= $href ?>join?code=<?= $user['meeting_code'] ?>";
                                                        if (navigator.share) {
                                                            navigator.share({
                                            ...
                                            <div class="col-span-12 md:col-span-4">
                                                <div class="grid grid-cols-2 mt-2">
                                                    <div>
                                                        <h5 class="text-15 leading-4 mb-2 font-medium text-gray-700 dark:text-white">Start Meeting</h5>
                                                        <p class="text-13 font-normal text-gray-600 dark:text-zink-200">Click the button below to start a meeting</p>
                                                    </div>
                                                </div>
                                                <button type="button" class="py-1 mt-6 border-transparent px-2 text-[11px] btn bg-blue-500 hover:bg-blue-600 text-white" onclick="startMeeting()">Start Meeting</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="dark:bg-zink-700">
                                    <div class="grid sm:grid-cols-3 gap-2" style="height: 90vh;">
                                        <div id="meet" class="card sm:col-span-2 px-3 flex flex-col min-h-96" style="width: 100%;">
                                            <?php if ($hls && $hls['hls']): ?>
                                                <!-- HLS Video Player -->
                                                <video id="hlsPlayer" class="w-full flex-1 object-cover" controls autoplay>
                                                    <source src="<?= htmlspecialchars($hls['hls']) ?>" type="application/x-mpegURL">
                                                ...
                                                <script>
                                                    if (Hls.isSupported()) {
                                                        ...
                                                        const hls = new Hls();
                                                        hls.loadSource('<?= htmlspecialchars($hls['hls']) ?>');
                                            ...
                                            <?php else: ?>
                                                <div id="start" class="flex items-center flex-col justify-center gap-2" <?php if ($meeting) echo 'style="display: none;"'; ?>>
                                                    <img src="https://cdni.iconscout.com/illustration/premium/thumb/business-meeting-illustration-download-in-svg-png-gif-file-formats--discussion-laptop-desk-team-pack-illustrations-2289786.png" class="h-38" alt="">
                                                    <p>Click the button below to start a meeting and share your link for people to join you</p>
                                                    <button class="bg-blue-500 rounded-lg py-3 px-8 text-white" onclick="startMeeting()">Start Meeting</button>
                                                </div>
                                            <?php endif; ?>

                                            <div class="flex justify-center gap-3 items-center gap-3 py-4">
                                                <button class="bg-blue-500 rounded-lg py-3 px-8 text-white" onclick="startMeeting()">Start Prayer</button>
                                                <a href="https://christembassy-ism.org/onlinepay/seeds.php" class="bg-green-500 rounded-lg py-3 px-8 text-white">Give now</a>
                                                <a href="#prayerRequests" class="bg-red-500 rounded-lg py-3 px-8 text-white">Prayer Request</a>
                                            </div>
                                        </div>
                                        <div class="card !pt-0 relative space-y-3 overflow-y-auto">
                                            <h5 class="text-15 font-medium text-white text-xl py-4 sm:py-8 bg-blue-500 text-center font-figtree border-b border-gray-200">Prayer Points</h5>
                                ...
                                <div class="carousel-container overflow-x-auto bg-blue-500 p-8">
                                    <div class="carousel-track flex transition-transform duration-1000 ease-in-out">
                                        <div class="carousel-slide min-w-full px-2">
                                            <img src="assets/images/prayernetwork-image.jpeg" alt="" class="w-full sm:h-96 object-cover rounded-lg">
                                        </div>
                                        <!-- <div class="carousel-slide min-w-full px-2">
                                            <img src="./assets/images/ismconf.jpg" alt="" class="w-full sm:h-96 object-cover rounded-lg">
                                        </div>
                                        <div class="carousel-slide min-w-full px-2">
                                            <img src="./assets/images/https://christembassy-ism.org/mopc/assets/resources/cover/conf-cover.jpg" alt="" class="w-full sm:h-96 object-cover rounded-lg">
                                        </div>
                                        <div class="carousel-slide min-w-full px-2">
                                            <img src="./assets/images/ismconf.jpg" alt="" class="w-full sm:h-96 object-cover rounded-lg">
                                        </div>
    ...
    <script>
        ...
        function startMeeting() {
            ...
            const meetingCode = "<?= $user['meeting_code'] ?>";
            ...
            // Show loading state
            if (startDiv) {
                startDiv.innerHTML = '<p>Starting meeting...</p>';
            }
        ...
        function closeMeeting() {
            fetch('./close.php', {
                ...
                .catch((error) => {
                    console.error('Error closing the meeting:', error);
                    window.location.reload();
                });
        }
    </script>
    ...
    <?php if ($meeting_live): ?>
        <script>
            // Ensure this runs after all other scripts by using window.onload
            window.onload = function() {
                // Small delay to ensure everything else has initialized
                setTimeout(function() {
                    const continueSession = confirm('You have an ongoing meeting that hasn\'t been closed. Would you like to continue with this meeting?');
                    if (continueSession) {
                        startMeeting();
                    } else {
                        // If user doesn't want to continue, close the meeting
                        closeMeeting();
                    }
                }, 100);
            };
    ...
    <!-- <script>
        // Auto-scroll for testimonies
        const container = document.getElementById('testimonies-container');
        const items = container.getElementsByClassName('testimony-item');
        let currentIndex = 0;
        ...
        setInterval(scrollToNextItem, 1000);

        // Auto-scroll for prayer requests
        const prayerContainer = document.getElementById('prayer-requests-container');
        const prayerItems = prayerContainer.getElementsByClassName('prayer-request-item');
        let prayerCurrentIndex = 0;

        function scrollToNextPrayerItem() {
            if (prayerItems.length === 0) return;

            prayerCurrentIndex = (prayerCurrentIndex + 1) % prayerItems.length;
            prayerContainer.scrollTo({
                top: prayerItems[prayerCurrentIndex].offsetTop,
                behavior: 'smooth'
            });
        }

        // Auto scroll every 5 seconds
        setInterval(scrollToNextPrayerItem, 1000);
    </script> -->
