  ...
  return (
    <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
      <View className="p-5">
              ...
              .map((ticket) => (
                ...
                >
                  <View className="flex-1">
                    <ThemedText className="font-semibold text-gray-800 dark:text-gray-300 capitalize">
                      {ticket.name}
                    </ThemedText>
        ...
        {/* Promo Code */}
        <View className="mb-5">
          <TouchableOpacity onPress={() => setIsPromoOpen(!isPromoOpen)} className="flex-row items-center gap-2">
          ...
          {isPromoOpen && (
            <View className="mt-3">
              {promoApplied ? (
                <View className="flex-row items-center justify-between bg-green-50 border border-green-200 dark:bg-green-900/20 rounded-xl p-4">
                  <View className="flex-row items-center gap-2">
                    <Ionicons name="checkmark-circle" size={20} color="#16a34a" />
                    <ThemedText className="text-green-600 font-semibold">{promoCode}</ThemedText>
                  </View>
                ...
                <>
                  ...
                  {promoError && (
                    <ThemedText className="text-red-500 text-xs mt-2">{promoError}</ThemedText>
                  )}
                </>
              )}
            </View>
        ...
        <View className="bg-white dark:bg-dark-bg rounded-xl border border-gray-100 dark:border-gray-700 p-4 gap-3">
          ...
          {discount > 0 && (
            <View className="flex-row justify-between items-center">
              ...
              <Currency className="font-semibold text-red-500">{discount}</Currency>
            </View>
          )}
